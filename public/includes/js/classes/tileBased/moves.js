...
var Path_Finder_Class = function(game, unit, x, y, moves, from, dir)
{
	...
	{
		if(moves==0)return;
		if(occupied.Player!=unit.Player)
		if(!game.Terrain_Map.At(x,y).Hidden)
		if(occupied.Alpha.data>0)return;
	}
	var back = from;
	while(back!=null)
	{
...
var Path_Finder_Handler = function(game, unit, x, y, onlyAllowedMoves)
{
	...
	if(unit.Range.length!=0)
		attack_offsets = Core.Target.Diamond(unit.Range[1], unit.Range[0], false);
	...
	{
		if(onlyAllowedMoves) // move not allowed because space is occupied, however, can be displayed if onlyAllowedMoves is not required
		if(game.Units_Map.At(x, y)!=null)
		if(game.Units_Map.At(x, y)!=unit)
		if(!game.Terrain_Map.At(x, y).Hidden)
		{
			// console.log(unit.Name,"space occ",x,y);
			return;
		}
...
var Move_Class = function(unit, start_x, start_y, terrain, changed)
{
	...
	{ // only ever called if X,Y is reachable; will change main path to shortest route
	...
	this.Add = function(x, y)
	{
		...
		if(line_of_tiles[squares][0]==x&&line_of_tiles[squares][1]==y)return true;
		if(!unit.Current_Path().Can_Move(x,y))
		{ // if location is attacking enemy or tile the unit can't reach
			if(unit.Slow_Attack)
				this.Wipe(); // if location is enemy, and the unit can't move when attacking
			// shortest(x, y);
			return true;
		}
		...
		{ // if it is adjacent
			...
			if(changed!=null) // if there is a callback function
				changed(unit, line_of_tiles); // usually used for displaying updated tiles
			return true;
		}
		else
		{ // if not adjacent, find shortest path to location
			shortest(x, y);
			return true;
		}
